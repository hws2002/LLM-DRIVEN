"""LLM client implementations for multiple providers (OpenAI, Z.AI, Qwen, Groq, Gemini)."""
...
try:
    ...
    for parent in [current_dir.parent.parent.parent, current_dir.parent.parent.parent.parent]:
        env_file = parent / '.env'
        if env_file.exists():
...
# API Configuration
# Prioritize DEV_OPENAI_API_KEY (for development), fallback to OPENAI_API_KEY for compatibility
OPENAI_API_KEY = os.getenv("DEV_OPENAI_API_KEY") or os.getenv("OPENAI_API_KEY")
OPENAI_API_URL = os.getenv("DEV_OPENAI_API_URL") or os.getenv("OPENAI_API_URL", "https://api.openai.com/v1")
...
class OpenAICompatibleClient(BaseLLMClient):
        ...
        while True:
            ...
            try:
                if self._uses_responses_api:
                    ...
                    self.last_usage = self._extract_usage_dict(getattr(completion, "usage", None))
    ...
    def _call_responses_api(
        ...
        *,
        ...
        self.last_usage = self._extract_usage_dict(getattr(response, "usage", None))
        ...
        if not output:
            print(f"⚠️ [DEBUG] Empty response from LLM. Raw response object: {response}")
            return ""
    ...
    def _extract_usage_dict(usage: Any) -> Optional[Dict[str, Any]]:
        ...
        if usage_dict:
            ...
            total_tokens = usage_dict.get("total_tokens", total_tokens)
            prompt_tokens = usage_dict.get("prompt_tokens", prompt_tokens)
            completion_tokens = usage_dict.get("completion_tokens", completion_tokens)
...
class GeminiClient(BaseLLMClient):
    ...
    def call_llm(
        ...
        try:
            ...
            while True:
                ...
                self.last_usage = self._extract_usage_dict(
    ...
    @staticmethod
    def _extract_usage_dict(usage_metadata) -> Optional[Dict[str, Any]]:
...
def create_llm_client(
    ...
    Args:
        provider: Provider name ("openai", "z.ai", "qwen", "groq", or "gemini")
    ...
    if provider == "openai":
        ...
        raise ValueError(
            f"Unknown provider: {provider}. Supported providers: openai, z.ai, qwen, groq, gemini"
        )
