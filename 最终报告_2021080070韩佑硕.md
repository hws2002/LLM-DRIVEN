# GraphNode —— Your next second brain
...
**子聚类。** 在大聚类内部，子聚类初始以聚合形式呈现（每个子聚类显示其规模与内部密度）；展开后即可看到子聚类内部的各个节点及其连边，实现从"全局鸟瞰"到"局部细节"的逐级下钻。
...
GraphNode 的知识不是存在单一数据库里，而是按"用途"拆分到三类存储：

| 存储 | 技术 | 存什么 | 作用 |
|---|---|---|---|
| **图数据库** | Neo4j | 实体节点、类型化的边、chunk 节点，以及 **实体↔chunk 的关联** | 表达知识的结构关系，支撑图谱遍历与 GraphRAG 的"图扩展" |
| **向量数据库** | ChromaDB | chunk 文本嵌入、节点嵌入 | 语义相似度检索（向量召回） |
...
目前，GraphNode 已实现一套可端到端运行的 AI 知识图谱处理流程，主要包括以下功能：

- **AI 对话与笔记输入。** 系统支持用户在平台内进行 AI 对话，也支持将已有对话记录、个人笔记或文档作为知识图谱生成的输入数据。
- **Macro Graph 生成。** 系统能够基于用户的全部对话与笔记，自动生成全局知识图谱，并以 Cluster、Sub-cluster、Node 的层次结构展示不同主题之间的关系。
- **Update Graph 增量更新。** 当用户产生新的对话或笔记后，系统可以分析其语义内容，并将其加入已有 Macro Graph 中，自动建立与旧节点之间的关联。
- **Microscope 单篇分析。** 对于单篇对话、笔记或文档，系统可以抽取其中的概念与关系，生成细粒度的 Micro Graph，帮助用户理解该文档内部的知识结构。
- **GraphNode Agent 问答。** 系统支持基于 Macro Graph 或 Micro Graph 中的节点进行问答，通过 GraphRAG 检索相关上下文并生成回答。
- **多格式与跨平台支持。** 当前系统已支持 AI 对话、Markdown 笔记以及部分文档格式的解析，并已完成与 Notion 的同步能力。

项目已部署在线预览版本：

- 在线预览：https://www.graphnode.site/app
- 源码位置：https://github.com/hws2002/LLM-DRIVEN/tree/main/codes/GraphNode_AI

在线版本可用于体验 AI 对话、笔记管理、Macro Graph、Microscope 分析以及基于图谱的 Agent 问答等主要功能。由于部分功能依赖外部 API Key、数据库和 SQS 队列服务，线上运行环境与本地源码审阅环境存在差异；源码中的核心模块也可以通过 `macro/src/run_pipeline.py`、`microscope/call.py`、`microscope/services/rag_service.py` 等入口单独查看或调试。
