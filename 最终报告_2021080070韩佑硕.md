...
GraphNode 的能力不止于可视化展示。在任意 Cluster / Macro / Micro 图谱中**点击节点**，即可唤起内置的 **GraphNode Agent**，基于该**图谱模块（Macro / Micro 节点，边）**进行问答。
...
本项中**LLM 并非系统的一个附加功能，而是贯穿整个软件的"知识结构化引擎"。** 软件的每一个核心环节都由大模型驱动，如下表所示。
...
- **场景三——Microscope 深度解析（重塑研究脉络）：** 将繁杂的课程笔记中的核心概念及其关系，在短暂的时间内自动结构化为清晰的知识脉络。

## 6. 技术实现与工程要点

本节介绍 GraphNode 的几个关键技术实现：数据的存储方式、GraphRAG 的工作原理、Macro 流水线的构成，以及相关的工程设计。具体代码位置与所用 Prompt 见**附录 A / B**。

### 6.1 三重存储架构（图 + 向量 + 文档）

GraphNode 的知识不是存在单一数据库里，而是按"用途"拆分到三类存储，由统一句柄 `GraphNodeDBHandler`（`infra/repositories/graph/graphnode_repository.py`）封装：

| 存储 | 技术 | 存什么 | 作用 |
|---|---|---|---|
| **图数据库** | Neo4j | 实体节点、类型化的边、chunk 节点，以及 **实体↔chunk 的关联** | 表达知识的结构关系，支撑图谱遍历与 GraphRAG 的"图扩展" |
| **向量数据库** | ChromaDB | chunk 文本嵌入、节点嵌入（`chunks_store` / `macro_node_store` 等） | 语义相似度检索（向量召回） |
| **文档数据库** | MongoDB | 原始对话与笔记 | 按 `node_id` 读取源文档（生产环境的抽取入口） |

关键设计是**单一写入路径**：所有图谱写入都汇聚到 `store_standardized_data()`，它在一次调用中把 chunk 写入 ChromaDB、把实体/边/chunk 节点写入 Neo4j，并通过 `link_entity_to_chunk` 建立"实体 ← 来源 chunk"的链接，从而保证图与向量两套存储始终一致。正是这条链接，让后续的 GraphRAG 能"由实体反查证据 chunk"。

### 6.2 GraphRAG：为什么本系统的 RAG 不止于向量检索

这是 GraphNode 在技术上最具特色的一环。普通 RAG 只做"向量相似度召回"，而 GraphNode 因为在 **Microscope 抽取阶段就把文档结构化成了图**，得以实现 **GraphRAG**。其成立依赖两个前提：

1. **抽取即结构化。** Microscope（`microscope/call.py`）先将文档 `chunk_document` 分块，再由 LLM 依据本体 Schema 从每个 chunk 中抽取 **node（实体）与 edge（关系）**（见附录 A.2）。因此落库后，文档不再是一堆孤立文本块，而是一张"实体—关系"图，且每个实体都链接着它的来源 chunk。
2. **检索时融合"语义"与"结构"。** RAG 检索（`microscope/rag/retrieval_strategies.py::retrieve_with_graph_expansion`）的流程为：

> ① 向量召回 top-k 初始 chunk（ChromaDB）→ ② 从中提取 `entity_names` → ③ 在 Neo4j 中对这些实体做 **1～2 hop 邻居扩展** → ④ 取邻居实体关联的 chunk → ⑤ 按 uuid 去重合并为上下文 → ⑥ 交 LLM 生成答案。

也就是说，召回的上下文不仅包含"与问题语义相近"的内容，还包含"在知识图谱上与之相邻"的内容——这正是 GraphRAG 相比普通向量 RAG 的关键优势：能顺着概念间的依赖、推导、关联关系把上下文"延展开"，而非只看孤立的相似片段。

在此基础上，系统还可加载用户的 Macro 画像（兴趣、学习风格、高频模式），注入到 RAG 提示词中（`microscope/rag/macro_context.py`），使回答贴合个人语境（实现细节见附录 A.3.2）。

### 6.3 Macro 流水线的工程实现

Macro（`macro/src/run_pipeline.py`）以多阶段流水线把"全部对话/笔记"端到端地构建为全局图谱：

> 嵌入 + 关键词提取 → LLM 聚类（大聚类生成 + 节点分配）→ 余弦相似度建边 → 合并 → 后处理 → Louvain 子聚类 → 嵌入索引（供检索）

其中**确定性任务**（嵌入、相似度、社区发现）用经典算法保证稳定与可复现：嵌入采用多语言 `sentence-transformers`，关键词用 KeyBERT，建边用余弦相似度的双阈值，子聚类用 Louvain 社区发现；而**语义任务**（聚类命名、节点归类、模式与建议）交由 LLM 完成。为应对 LLM 输出的不确定性，工程上做了多重稳健化：节点分配采用更难被截断的**纯文本管道格式**并对未完成项自动重试，聚类失败时回退到基于关键词相似度的规则分配（见附录 A.1）。

### 6.4 支撑性工程设计

- **依赖注入（DI）。** `GraphNodeDBHandler` 与 `ApiProvider` 在 worker 启动时只实例化一次，并以参数注入各流水线，使业务逻辑与数据库/LLM 细节解耦，便于测试与替换。
- **多 provider 统一封装。** `shared/api_provider.py` 将 OpenAI / Groq / Z.AI 等抽象为统一接口，可按环境变量切换模型与 provider，不影响上层代码。
- **异步 SQS Worker。** 服务以 `server/worker.py` 形式部署，异步消费 SQS 任务并支持多任务并发处理，结果经结果队列回传。
- **跨平台与多格式接入。** 数据接入侧支持 ChatGPT / Claude / Gemini 的对话历史导入与 docx / PDF / pptx / Markdown 解析；数据流转侧规划通过 MCP（Model Context Protocol）等机制与飞书、Notion、Obsidian 同步。
...
GraphNode 目前已是一套可端到端运行的系统。站在用户的角度，它带来的核心体验是：**把"聊过即忘"的零散对话，变成一座可以走进去、能持续生长的个人知识城市。**

- **一键看见自己的知识全貌。** 用户无需手动整理，导入或积累对话与笔记后，系统自动生成 Macro Graph——用户得以从"大聚类 → 子聚类 → 节点"逐级俯瞰自己学过、想过的一切，并发现其中反复出现的兴趣与盲区。
- **想深入时，随时放大到单篇。** 对任意一篇对话或笔记，用户可用 Microscope 在数分钟内得到其概念-关系图谱，快速吃透一篇复杂材料的内在逻辑。
- **基于自己的知识对话。** 点击任意节点即可发起 GraphNode Agent 问答；借助 GraphRAG，回答不仅检索语义相近的内容，还顺着图谱关联延展上下文，并融合用户的学习画像，给出贴合个人语境的解答。
- **知识随用随长。** 每次新对话或新笔记都能通过 Update Graph 自动归位到既有图谱中，使这座"知识城市"与用户共同成长，而非一次性快照。

简言之，用户得到的不是又一个聊天框或又一个笔记本，而是一个**会自动整理、能可视化、可对话、持续积累**的第二大脑。

### 7.2 需求验证

为确认上述体验确为真实需求，项目对清华大学与北京大学的留学生开展了问卷调研：

- **69.2%** 的受访者将"难以找回过往笔记的上下文"视为现有工具的最大痛点；
- **100%** 的受访者将"学习与研究整理"作为核心使用场景；
- **92.3%** 的受访者将"知识可视化"视为最期待的功能。

这些数据与 GraphNode 所提供的体验高度吻合，印证了"自动结构化 + 可视化 + 基于图谱问答"这一方向的价值。

### 7.3 未来发展方向

未来的演进同样围绕"为用户创造更主动、更无缝的知识体验"展开：

- **从"应答"到"配速员（Pace-maker）"。** 让 Agent 主动结合用户最近的对话、情境与目标（如备考、求职准备），充当并肩同行的学习伙伴，主动提示进度与下一步，而不只是被动回答。
- **Agentic Knowledge Management。** 将目前相互独立的能力（Macro 生成、Update、Microscope）整合为统一的 Agent 工作流——用户一句"帮我整理上个月关于机器学习的对话"，Agent 即自动完成检索、聚类、深度解析与综合摘要，把多步操作收敛为一次自然语言指令。
- **多模态与跨平台无缝接入。** 在文本之外纳入图片、PDF、课程视频等多模态输入；并在已跑通 Notion 同步的基础上深度接入 Obsidian、飞书等工具，让用户在现有工作流中即可享有 GraphNode 的能力。
...
**代码：** `microscope/rag/macro_context.py`+ `rag/prompt_builder.py::rag_with_profile`。
...
由此，**同一个 Micro RAG 在拥有 Macro 摘要时，会给出贴合个人语境的回答**；
...
| Microscope Schema | `microscope/schema/ontology_schema_general.json` |
